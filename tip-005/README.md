...
For the method name there's even more choices. We can name our method using one of the following names (+ Async suffix): Handle, Handles, Consume, Consumes, Orchestrate, Orchestrates, Start, Starts, StartOrHandles, NotFound. Again if none of those works for us we can use `[WolverineHandler]` attribute.
